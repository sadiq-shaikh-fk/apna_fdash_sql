...
---------- table rfp_assets ----------
CREATE TABLE rfp_assets (
  rfpa_id BIGSERIAL PRIMARY KEY,
  rfpa_rfp_id INTEGER NOT NULL,    -- foreign key to 'rfp_id' from rfp table
  rfpa_assets TEXT,
  rfpa_t_id INTEGER,
  -- audit and logs
  created_by VARCHAR(100) NOT NULL DEFAULT current_user,
  ...
  deleted_by VARCHAR(100),
  -- Foreign key constraints
  CONSTRAINT fk_rfps_assets FOREIGN KEY (rfpa_rfp_id) REFERENCES rfp(rfp_id) ON DELETE RESTRICT,
  CONSTRAINT fk_rfpa_assets_tenant FOREIGN KEY (rfpa_t_id) REFERENCES tenants(t_id) ON DELETE RESTRICT
);
...
CREATE TABLE influencer_bidding (
  ...
  CONSTRAINT fk_influencer_bidding_influencer_socials FOREIGN KEY (ib_is_id) REFERENCES influencers_socials(is_id) ON DELETE RESTRICT,
...
CREATE TABLE rfp_cart_details (
  ...
  deleted_by VARCHAR(100),
  -- composite unique constraint
  CONSTRAINT unique_cart_per_user UNIQUE (rfp_cr_name, created_by),
  -- Foreign key constraints
...
CREATE TABLE rfp_cart_items (
  ...
  -- Prevent duplicate deliverables in same cart
  CONSTRAINT unique_deliverable_per_cart UNIQUE (rfp_ci_rfp_cr_id, rfp_ci_ib_id)
);
