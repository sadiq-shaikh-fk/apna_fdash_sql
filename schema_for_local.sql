...
CREATE TABLE organizations (
  ...
  o_plan_id INTEGER NOT NULL,    -- foreign key to 'plan_id' from plans table
  ...
  deleted_by VARCHAR(100),
  -- Foreign key constraints
  CONSTRAINT fk_organization_o_plan_id FOREIGN KEY (o_plan_id) REFERENCES plans(plan_id) ON DELETE RESTRICT,
  -- constraints
...
CREATE TABLE subscriptions (
  ...
  sub_o_id INTEGER NOT NULL,     -- foreign key to 'o_id' from organizations table
...
-- audit and logs
  ...
  deleted_by VARCHAR(100),
  -- Foreign key constraints
  CONSTRAINT fk_subscriptions_sub_o_id FOREIGN KEY (sub_o_id) REFERENCES organizations(o_id) ON DELETE RESTRICT,
...
-- audit and logs
  ...
  deleted_by VARCHAR(100),
  -- Foreign key
  CONSTRAINT fk_invoices_i_org_id FOREIGN KEY (i_org_id) REFERENCES organizations(o_id) ON DELETE RESTRICT
);
...
CREATE TABLE payments (
  ...
  CONSTRAINT fk_payments_pay_org_id FOREIGN KEY (pay_org_id) REFERENCES organizations(o_id) ON DELETE RESTRICT,
  -- Unique constraint
...
CREATE TABLE brands (
  ...
  b_o_id INTEGER,   -- foreign key to 'o_id' from organizations table
  b_t_id INTEGER NOT NULL,    -- foreign key to 't_id' from tenants table
  -- audit and logs
  created_by VARCHAR(100) NOT NULL DEFAULT current_user,
  created_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT current_timestamp,
  modified_by VARCHAR(100) NOT NULL DEFAULT current_user,
